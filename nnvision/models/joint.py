...
def multi_task_models_gauss_readout(
    dataloaders: Dict[str, Any],
    seed: int,
    input_channels: int = 1,
    model_names_1: Union[str, List[str]] = "vgg19",  # begin of core_1 args
    layer_names_1: Union[str, List[str]] = "features.10",
    pretrained_1: Union[bool, List[bool]] = True,
    biases_1: Union[bool, List[bool]] = False,
    final_batchnorms_1: Union[bool, List[bool]] = True,
    final_nonlinearities_1: Union[bool, List[bool]] = True,
    momentums_1: Union[bool, List[bool]] = 0.1,
    fine_tunes_1: Union[bool, List[bool]] = False,
    model_name_2: str = "vgg19",  # begin of core_2 args
    layer_name_2: str = "features.10",
    pretrained_2: bool = True,
    bias_2: bool = False,
    ...
    """
    Concatenate the features of a single or multiple task-driven 
    cores with an extra core
    """
    ...
    # First core
    multiple = False
    if isinstance(model_names_1, list) or isinstance(model_names_1, tuple):
        l = len(model_names_1)
        multiple = True
        assert all([len(v) == l for v in (
            model_names_1, 
            layer_names_1, 
            pretrained_1, 
            biases_1, 
            final_batchnorms_1,
            final_nonlinearities_1,
            momentums_1,
            fine_tunes_1,
        )]), "Length of _1 vars should match"
    
    if multiple:
        cores = []
        for mn, ln, ptr, b, bn, fn, mom, ft in zip(
            model_names_1, 
            layer_names_1, 
            pretrained_1, 
            biases_1,
            ...
            core_here = TaskDrivenCore2(
                ...
                bias=b,
                final_batchnorm=bn,
                final_nonlinearity=fn,
                momentum=mom,
                fine_tune=ft,
            )
            set_random_seed(seed)
            core_here.initialize()
            cores.append(core_here)
        
        core_1 = JointCore(cores[0], cores[1])
    else:  
        set_random_seed(seed)
        core_1 = TaskDrivenCore2(
            input_channels=core_input_channels,
            model_name=model_names_1,
            layer_name=layer_names_1,
            pretrained=pretrained_1,
            bias=biases_1,
            final_batchnorm=final_batchnorms_1,
            final_nonlinearity=final_nonlinearity_1,
            momentum=momentums_1,
            fine_tune=fine_tunes_1,
        )
        set_random_seed(seed)
        core_1.initialize()
